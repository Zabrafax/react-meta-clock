...
export function UserProvider({ children }) {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                const response = await fetch(`${process.env.REACT_APP_API_BASE}/users/verify-token`, {
                    method: "POST",
                    headers: {
    ...
    const saveColorTheme = async (colorTheme) => {
        if(isLoggedIn) {
            ...
            try {
                const response = await fetch(`${process.env.REACT_APP_API_BASE}/users/save-color-theme`, {
                    method: "POST",
                    headers: {
                        "content-type": "application/json",
                        "Authorization": `Bearer ${token}`
    ...
    const saveTimeZone = async (timeZone) => {
        if(isLoggedIn) {
            ...
            try {
                const response = await fetch(`${process.env.REACT_APP_API_BASE}/users/save-timezone`, {
                    method: "POST",
                    headers: {
                        "content-type": "application/json",
                        "Authorization": `Bearer ${token}`
    ...
    const loginUser = async (username, password) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_BASE}/users/login`, {
                method: "POST",
    ...
    const registerUser = async (username, password, timeZone, colorTheme) => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.REACT_APP_API_BASE}/users/register`, {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({
                    username,
